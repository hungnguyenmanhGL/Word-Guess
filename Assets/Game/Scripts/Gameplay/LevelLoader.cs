...
public struct LoadLevelOption {
    ...
    [SerializeField] private TopicData data;
    ...
    public override string ToString() {
        return $"[LoadLevelOption] level= {level}, path= {path}, topic = {data}";
    }
    ...
    public static LoadLevelOption Create(TopicData topic) {
        string path = string.Format("Levels/Level_{0}", 1);
        LoadLevelOption option = new LoadLevelOption { level = 1, path = path, data = topic};
        return option;
    }
}
